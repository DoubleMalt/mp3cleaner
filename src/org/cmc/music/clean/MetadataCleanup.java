...
public class MetadataCleanup implements MusicMetadataConstants
{
	...
	private String removeQuotes(String s)
	{
		...
		if (Pattern.compile("^\".+\"$").matcher(s).matches() || Pattern.compile("^'.+'$").matcher(s).matches()
				|| Pattern.compile("^\\{.+\\}$").matcher(s).matches()
				|| Pattern.compile("^\\(.+\\)$").matcher(s).matches() || Pattern.compile("^<.+>$").matcher(s).matches()
				|| Pattern.compile("^\\[.+\\]$").matcher(s).matches())
		{
	...
	private String clean(String s, IMusicMetadata flags)
	{
...

		{
			...
			for (int i = 0; i < DISC_PatternGEXS.length; i++)
			{
				String kDISC_PatternGEX = DISC_PatternGEXS[i];

				Pattern re = Pattern.compile(kDISC_PatternGEX);
				Matcher match = re.matcher(s.toLowerCase());
				if (match.matches())
				{
	...
	private String removeTrackNumbers(String s, IMusicMetadata flags)
	{
		...
		if (Pattern.compile("^(audio)? ?track ?[- ]?[0-9][0-9]?$")
		...
		if (Pattern.compile("^piste ?[- ]?[0-9][0-9]?$").matcher(s.toLowerCase()).matches())
		{
			parseTrackNumber(s.substring(5), flags);
			// Debug.debug("discarding track...", s);
			return null;
		...
		if (Pattern.compile("^[0-9][0-9] - ").matcher(s).matches()
				|| Pattern.compile("^[0-9][0-9][0-9] - ").matcher(s).matches()
				|| Pattern.compile("^[aAbBcCdD][0-9] - ").matcher(s).matches())
		{
		...
		if (Pattern.compile("^\\([0-9][0-9]\\) ").matcher(s).matches()
				|| Pattern.compile("^\\([abcdABCD][0-9]\\) ").matcher(s).matches())
		{
			// Debug.debug("attempting to strip track number...", s);
			int index = s.indexOf(')');
			if (index >= 0)
	...
	private String removeYearPrefixSuffix(String s)
	{
		...
		if (Pattern.compile("^\\(199[0-9]\\)").matcher(s).matches()
				|| Pattern.compile("^\\(200[0-9]\\)").matcher(s).matches())
			s = s.substring(7);
		...
		if (Pattern.compile("199[0-9] - ").matcher(s).matches() || Pattern.compile("200[0-9] - ").matcher(s).matches())
		{
			int index = s.indexOf('-');
			if (index >= 0)
			{
	...
	private String removeURLs(String s)
	{
		...
		{
			if (Pattern.compile("^http://").matcher(s.toLowerCase()).matches())
				return null;
			// if (Pattern.compile("^[hH][tT][tT][pP]://").matcher(s).matches())
			// return null;
		}

		{
			...
			// Debug.debug("s1", s);
			Pattern re = Pattern.compile("^[\\w \\-]*\\.[\\w \\.\\-]*\\.(com|net|org|edu)$");
			// re.setMatchFlags(Pattern.MATCH_CASEINDEPENDENT);
			if (re.matcher(temp.toLowerCase()).matches())
				return null;
	...
	private Vector listToNames(Matcher match)
	{
		Vector result = new Vector();
		...
		{
			String child = match.group(i * 2 + 1);
	...
	private Vector debugPattern(Matcher match)
	{
		Vector result = new Vector();
			...
			Vector featured_artists, String pattern)
	{
		...
		if (match.matches())
		{
			...
			int startInside = match.start(1);
			// Debug.debug("\t" + "startInside", startInside);
			int endInside = match.end(1);
			...
			Matcher match2 = re2.matcher(right);
			if (match2.matches())
			{
				Vector v = listToNames(match2);
				featured_artists.removeAll(v);
				featured_artists.addAll(v);
			} else
			{
				re2 = Pattern.compile("(.*)(,.*)*\\ And (.*)");
				if (match2.matches())
				{
					Vector v = listToNames(match2);
					featured_artists.removeAll(v);
	...
	// private static final String kPatternGEX_OPEN_TOKENS = "\\(\\[\\{\\\"\\'";
	// private static final String kPatternGEX_CLOSE_TOKENS = "\\)\\]\\}\\\"\\'";
	...
	public String getPrefixPattern(String s, boolean permissive)
	{
		return "^('" + s + "'|\\\"" + s + "\\\"|\\[" + s + "\\]|\\(" + s
				+ "\\)|\\{" + s + "\\}|" + s + "\\-"
				+ (permissive ? "|" + s + " " : "") + ")";
		// return ("^[" + kPatternGEX_OPEN_TOKENS + "]?" + s + " ?["
		// + (permissive ? " " : "") + "\\-" + kPatternGEX_CLOSE_TOKENS + "]");
	}
	...
	public String getSuffixPattern(String s, boolean permissive)
	{
		return "('" + s + "'|\\\"" + s + "\\\"|\\[" + s + "\\]|\\(" + s
				+ "\\)|\\{" + s + "\\}|\\-" + s + ""
				+ (permissive ? "| " + s : "") + ")$";
		// return ("[" + (permissive ? " " : "") + "\\-" + kPatternGEX_OPEN_TOKENS
		// + "] ?" + s + "[" + kPatternGEX_CLOSE_TOKENS + "]?$");
	}
	...
	public String getPrefixPattern2(String s)
	{
		return "^('.*'|\\\".*\\\"|\\[.*\\]|\\(.*\\)|\\{.*\\}|.*\\-) ?" + s
				+ "$";
		// return ("^[" + kPatternGEX_OPEN_TOKENS + "]?" + "(.*)" + " ?["
		// // + (permissive ? " " : "")
		// + "\\-" + kPatternGEX_CLOSE_TOKENS + "] ?" + s + "$");
	}
	...
	public String getSuffixPattern2(String s)
	{
		...
		// return ("^" + s + " ?["
		// // + (permissive ? " " : "")
		// + "\\-" + kPatternGEX_OPEN_TOKENS + "] ?" + "(.*)" + "["
		// + kPatternGEX_CLOSE_TOKENS + "]?$");
	}
	...
	private String stripRegexMatch(String s, String pattern)
	{
		...
		try
		{
			...
			Pattern re = PatternGEX_CACHE.getRegEx(pattern);
			// Pattern re = Pattern.compile(pattern.toLowerCase());
			...
			s = s.substring(0, match.start(0))
					+ s.substring(match.end(0));
	...
	private static class RegExCache
	{
		...
		public final Pattern getRegEx(String pattern)
		{
			if (pattern == null)
				return null;
			pattern = pattern.toLowerCase();
			Pattern result = (Pattern) map.get(pattern);
			if (result == null)
			{
	...
	private static final RegExCache PatternGEX_CACHE = new RegExCache();
